...
module Cyclopedio
  module Syntax
    class PennTree
      ...
      # Fix the number assigned to the tree nodes using the
      # plural/singular data from the +nouns+ library.
      # It changes the tree in-place and returns the number of changes.
      #
      # The library has to respond to two basic methods:
      # - singular?(word_form) - returns true if the word form is singular
      # - plural?(word_form)- returns true if the word form is plural
      def fix_number(nouns)
        changes = 0
        first = true
        self.each_leaf do |l|
          if l.parent.content =~ /^NNP?S=H$/ # plural TODO =H
            word = l.content
            if !nouns.plural?(word) && !(first && nouns.plural?(word.downcase)) &&
                (nouns.singular?(word) || (first && nouns.singular?(word.downcase)))
